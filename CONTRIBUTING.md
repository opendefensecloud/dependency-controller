...
Thanks for contributing. This repository provides a kcp controller for resource
deletion protection in a dependency tree.
...
1. Open an issue, or find an open one. New issues carry `needs-triage` until a
   maintainer has looked at them — wait for triage before starting.
...
PRs land as merge commits — squash and rebase are disabled — so every commit in
your branch keeps its own message and gets linted. Tidy the branch before
review rather than leaving `fixup` commits for the merge to swallow.
...
`direnv allow` (or `nix develop`) gives you a shell with the toolchain on
`$PATH`; `pre-commit install` registers the hooks. Then:

```bash
make lint        # golangci-lint plus license headers
make test        # unit and integration tests; KCP_VERSION=<x.y.z> picks the kcp version
make test-e2e    # kind + kcp + helm, creates its own cluster (needs docker)
```

`make test-matrix` and `make test-e2e-kcp-matrix` run the same suites across
every supported kcp version. `make help` lists the rest, and
[docs/development.md](docs/development.md) covers the test internals.
...
| Golang CI | Go sources, `go.mod`, `go.sum`, `Makefile`, `flake.nix` | `golangci-lint`, license headers, `go build`, `go vet`, and `make test` against every kcp version in [docs/compatibility.md](docs/compatibility.md) |
| Helm Lint | `charts/**` | `helm lint` over each chart |
| Docker build | `ok-to-image` label | Builds the container image. A maintainer adds the label |
...
than merging past a red one.

## Getting merged

Beyond green checks, the `main` ruleset requires:

- one approving review, including a
  [CODEOWNERS](.github/CODEOWNERS) approval
...
container image, and a published GitHub release (or a `v*` tag) publishes both
the image and the Helm chart.
...
dependency-controller, is not accepted.
